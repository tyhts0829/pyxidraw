...
- [x] PipelineBuilder/キャッシュの分離と失効条件の明確化
  - [x] UI 介在、ラベル管理、キャッシュ設定を分離した責務分担案をまとめる
  - [x] `_GLOBAL_COMPILED` のキーに含めるべき構成要素（impl_id/設定バージョン等）を定義
  - [x] `run_sketch` 起動・終了などライフサイクルでのキャッシュクリア手順を設計
  - [x] 互換のためのインターフェース（現行チェーン API）維持策を決める
...
- [x] `run_sketch` のオーケストレーション分割
  ...
  - [x] 分割後の単体テスト可能な境界と依存注入（時計、ログ、エクスポート先）を定義

- [x] ドキュメント/設計同期
  - [x] `architecture.md` および関係 AGENTS.md に必要な更新箇所を特定
  - [x] 変更内容に合わせてテスト方針・禁止エッジを更新する案を用意
