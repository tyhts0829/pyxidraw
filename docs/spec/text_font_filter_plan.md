...
- 出力を JSON などの機械可読な一覧にまとめ（ファミリ名・パス・font_index・ソース種別を想定）、ソートして決定論的に生成する。`--dry-run` で書き込み無しの確認も可能にする。
- Text シェイプの `_font_choices` は生成済み出力を優先し、無い場合のみ従来の列挙にフォールバックする。表示名の決め方は出力のフィールドに従う。
- スクリプトの使い方と出力ファイルパスをドキュメント化し、手動実行を基本とする（CI での自動実行は想定しない）。

## 実装タスク チェックリスト（ドラフト）

- [ ] 必須グリフセット・出力パス・デフォルト検索範囲（config search_dirs と OS をどう扱うか）を決定し、仕様を固定する。
- [ ] `tools/<new_script>.py` の CLI 骨子を用意（argparse、`--search-dir`/`--include-os`/`--output`/`--required-glyphs`/`--dry-run` など）。安全側の既定値を設定。
- [ ] フォント検証ロジックを実装（fontTools で face を開き、指定コードポイントが `cmap` に揃っているか判定；読み込み失敗は握りつぶさずログへ）。
- [ ] 抽出結果の整形・出力（決定論的なソートと重複排除、family 表記と表示名の決定ルールを整理、JSON 書き出しと dry-run プレビューを実装）。
- [ ] `src/shapes/text._font_choices` を出力ファイル優先で読む形に変更し、欠損時は既存列挙へフォールバックする。データ構造が変わる場合は適宜ユーティリティを追加。
- [ ] ドキュメント更新（本計画の反映・スクリプトの実行手順・出力の配置と `.gitignore` 方針、必要なら `configs/default.yaml` のコメント追記）。
