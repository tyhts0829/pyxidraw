...
------------------
- ゴール
  - `LazyGeometry.__add__` と `__radd__` を実装し、`Lazy+Lazy` / `Lazy+Geometry` / `Geometry+Lazy` をサポート。
  - 演算子は“即時実体化しない”設計（完全遅延）。
  - 多項結合のコストを O(totalN) に抑える（繰り返しコピーを避ける）。
  - 既存の Prefix LRU と両立し、キャッシュヒット率を下げない。
- ノンゴール
  - 幾何ブール演算（和/差/積）ではなく、あくまで「ポリライン集合の連結」。
  - `__iadd__`（`+=`）は後続（必要なら追加）。
...
- 組合せ
  ...
  - `Lazy + Geometry` → 右項は参照で保持。実体化は realize 時に行う。
  - `Geometry + Lazy` → `LazyGeometry.__radd__` で処理（左項を基準に遅延連結）。
...
- `__add__/__radd__` は plan の末尾が `_fx_concat_many` なら右項をその `rhs_ref_list` に“集約”（フラット化）し、無ければ新規に1ステップ追加。
  - これにより `a + b + c + d` は「concat ステップ ×1」で表現でき、反復コピーを避ける。

2) 1パス結合（O(totalN））
- `_fx_concat_many` は以下で一括結合する:
  - `g` と `rhs_ref_list` の各要素（`Geometry` または `LazyGeometry`）を必要時に `realize()`。
  - 総頂点数と総線本数を先に算出→出力配列を一括確保。
  - `np.vstack/hstack` の多段使用を避け、プレ割り当て配列へブロックコピー＋`offsets` シフトを一回で実施。

3) キャッシュ（Prefix LRU）と署名
- 署名づけ用に、右項ごとに以下のシグネチャを作成し、`rhs_sig_list: tuple[bytes|tuple, ...]` としてエフェクトパラメータに格納。
  - `LazyGeometry` → `api.lazy_signature.lazy_signature_for(lg)`（決定的）
  - `Geometry` → `("geom-id", id(g))`（プロセス内同一性）。
...
- `LazyGeometry.realize()` 内の「effect 署名生成」箇所を小変更し、`eparams` から以下を除外した dict を `params_signature()` に渡す:
  - キーが `_ref` で終わる項目（例: `rhs_ref_list`）
  - または値が `ObjectRef` の項目
...
- src/engine/core/lazy_geometry.py
  - [ ] `class LazyGeometry` に `__add__` / `__radd__` を追加。
    - 右項を正規化（`Geometry` or `LazyGeometry` のみ許容）。
    - 末尾が `_fx_concat_many` の場合はそこへ `rhs` を追記、無ければ新規ステップ追加。
  - [ ] トップレベルに `_fx_concat_many(g, *, rhs_sig_list, rhs_ref_list)` を追加。
    - `rhs_ref_list` は `ObjectRef` の列。適用時に `unwrap()` して実体を取得・実体化。
    - 1 パスで coords/offsets を構築。
  - [ ] `realize()` の署名生成箇所を修正（`_ref` キーと `ObjectRef` 値を除外して計算）。

- architecture.md（LazyGeometry セクション）
  - [ ] 演算子方針と concat 集約の挙動を追記。

- テスト（tests/ 以下、新規）
  - [ ] `test_lazy_add_basic`: `Lazy+Lazy`/`Lazy+Geometry`/`Geometry+Lazy` の結果が `Geometry.concat` と一致。
...
- 署名パラメータ例（`_fx_concat_many`）
  - `eparams = {"rhs_sig_list": (b"...", ("geom-id", 12345)), "rhs_ref_list": [ObjectRef(lg), ObjectRef(g)]}`
...
- リスク: 署名生成の変更が既存エフェクトに影響
  - 対策: 除外条件を「キーが `_ref` で終わる」＋「値が `ObjectRef`」に限定（保守的）。
- リスク: 大量結合で一時メモリピーク
...
確認事項（要ご判断）
--------------------
- [ ] `__iadd__`（`+=`）も同時に対応しますか？（API 完結性 vs 先送り）
- [ ] 署名計算での除外規則（`_ref` と `ObjectRef`）は上記方針で問題ありませんか？
- [ ] 右項 `Geometry` のシグネチャを `id` 以外（例: クイックダイジェスト）にしますか？（コストとトレードオフ）
- [ ] 連結ステップの集約（フラット化）閾値は常時有効で良いですか？（分岐なし想定）
