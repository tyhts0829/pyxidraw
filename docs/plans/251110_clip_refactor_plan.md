...
- [x] 未使用・冗長コードの整理（安全な削除）
  - [x] `all_offsets` と関連ロジックの削除（最終的に `offs` 再計算で代替済み）
  - [x] `coord_shift` の削除（未使用）
  - [x] フォールバック内 `xs` の削除（コメント含め未使用）
  - [x] `_segment_intersections_with_polygon_edges` 戻り値 `(t, x)` → `(t)` に簡素化（呼び出し側未使用の `x` を除去）

- [x] 2D クローズ処理の簡素化
  - [x] `_prepare_projection_mask` の 2D 環での閉路処理を 3D 経由せず 2D 専用ヘルパへ（意図を明確化）

- [x] import の重複/ばらつき整理
  - [x] `import os as _os` の重複インポートを整理（先頭へ集約）

- [x] 仕様コメント/注記の微修正（挙動は変更しない）
  - [x] on-edge 取り扱い（Shapely 経路とフォールバックの差異があり得る）を短い注記として docstring に追記
  - [x] モード安定化（`_MODE_PIN`）の方針注記強化（初回モードのピン挙動）

- [x] 軽微な型/名前の一貫性
  - [x] 戻り値/引数の `np.ndarray` 型注釈の確認と不足時の補完（mypy 静的エラーが出ない範囲）
