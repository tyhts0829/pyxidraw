...
- 入力統合モデル（t/cc/gui は同一概念）
  - t/cc/gui を「ただの数値入力」として 1 つの InputContext に統合。ParameterRuntime は毎フレーム `set_inputs(t, cc_snapshot, gui_overrides)` を受け取り、各 shape/effect の引数値を決める唯一の参照源（ParameterStore）を更新する。
...
- [ ] Effects: `E.pipeline` を「宣言 + 実行」統合オブジェクトへ刷新（署名比較／裏ビルド／RLock／`.build()` は同義）。
- [ ] PipelineKey: 「解決後（量子化後）パラメータ」を材料に算出するよう変更。
- [ ] Shapes: Runtime 介在時も「解決後パラメータ」で `_cached_shape` を使うよう経路変更。
- [ ] Runtime: 入力スナップショット（t/cc/gui）を `set_inputs()` で受け、ValueResolver が ParameterStore の値を一元解決（original→midi_override→gui_override）。
...
段階0: 現状確認と影響範囲の固定
- [ ] `src/api/effects.py` を精読し、`Pipeline`/`PipelineBuilder` の責務と依存（`_geometry_hash`, `_fn_version`, `_params_digest`, `RLock`, `OrderedDict`）を洗い出す。
- [ ] `src/api/shapes.py` の Runtime 介在時 LRU バイパス箇所（200行前後）と `_cached_shape` 呼び出し経路を把握。
- [ ] `engine/ui/parameters`（`runtime.py`/`value_resolver.py`/`state.py`）で、値解決・`__param_meta__` 取得・override の流れを把握。
- [ ] `tools/gen_g_stubs.py` の E.pipeline/Builder/`.build()` の Stub 生成を確認（刷新時の同期対象）。
- [ ] ベンチ/テストのキャッシュ参照箇所を確認（`tests/perf/test_pipeline_perf.py`）。
...
- [ ] `ParameterRuntime` に `set_inputs(t, cc_snapshot, gui_overrides)` を追加。`t` は float、`cc_snapshot` は `{int: float}`、`gui_overrides` は `{param_id: Any}`。
- [ ] `ParameterManager.draw()` で `runtime.set_inputs(t, merged_cc, controller.get_gui_overrides())` を呼ぶ（GUI 側は簡易実装でも良い。必要なら `ParameterWindow` から overrides を引く API を追加）。
- [ ] ValueResolver は ParameterStore の `original`（ブループリント）に対し、`midi_override`（CC）と `override`（GUI）を順に適用して“現在値”を返す。`t` は関数が `t` 引数を持つ場合にのみ追加で渡す。

段階3: Shapes — Runtime 下でも LRU を有効化
- [ ] `ShapesAPI._build_shape_method()` を変更。
  - Runtime 有効時: `params_resolved = runtime.before_shape_call(...)` → `params_tuple = _params_to_tuple(params_resolved)` → `return _cached_shape(name, params_tuple)`。
  - 二重解決を避けるため、`_cached_shape()` が呼ぶ内部実行を Runtime 無し版（`_generate_shape_resolved(name, params_dict)`）へ切替。
- [ ] `_generate_shape()` を分割し、Runtime 無しで実行する経路を新設。ドキュメント/型注釈も更新。

段階4: Effects — `E.pipeline` の統合と裏ビルド
- [ ] `PipelineBuilder` と `Pipeline` を統合し、`Pipeline`（ファサード）に一本化。
  - ステップ列（ブループリント）と直近の「解決後署名」、内部実行体 `self._compiled: _CompiledPipeline | None` を保持。
  - `.build()` は no-op（`return self`）。`.strict()` は撤廃。`.cache()` は存続。
- [ ] `_CompiledPipeline`（内部クラス）を新設。
  - 役割: 現在の「解決後（量子化後）パラメータ」で固定化された実行体。従来どおり LRU を持ち、初期化時に `pipeline_key` を計算。
  - `__call__(g)` は従来実装を踏襲（Runtime 介在不要）。
- [ ] `Pipeline.__call__(g)` を刷新。
  - Runtime で各ステップの params を解決 → 量子化 → 署名タプル生成。
  - 直近署名と比較し、異なれば `_CompiledPipeline` を再生成（裏ビルド）して差し替え。同一なら再利用。
  - `RLock` で差し替えと参照を保護。
- [ ] `pipeline_key` 算出は「解決後（量子化後）パラメータ」を用いるよう変更（`_params_digest` の入力を署名由来に）。

段階5: API ・スタブ同期
- [ ] `api/__init__.pyi` の `Pipeline/Builder` 型を統合仕様へ更新（`.build()` は同義）。
- [ ] `tools/gen_g_stubs.py` の E.pipeline 生成部を更新。
- [ ] `.strict()` のスタブ/実装を削除（互換レイヤは設けない）。
...
- [ ] type/lint: 変更ファイルに限定し `ruff/black/isort/mypy` を通す。
