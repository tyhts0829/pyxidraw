...
GUI 表示と内部形式の方針（重要）
- GUI（ColorEdit）の表示・入力は 0–255 の整数 RGB（`no_alpha=True`）に固定。
- 内部保存・描画適用は 0–1 の RGBA。GUI 入出力時に共通パーサで相互変換する。

HUD コントロール（テキスト/メータ）
- HUD の色（Text/Meter/Meter BG）は Parameter GUI から操作できる。
- GUI は 0–255 の整数 RGB（alpha 不要）、内部は 0–1 RGBA（保存/復帰・描画適用）で統一。
