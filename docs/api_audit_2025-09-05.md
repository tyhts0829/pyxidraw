...
- [x] C1-displace-API: `effects/noise.py:displace` を新シグネチャのみに削減（`amplitude_mm`, `spatial_freq`, `t_sec`）。`intensity`/`frequency`/`time` と警告分岐を削除。`__param_meta__` を更新。
- [x] C2-pipeline-strict-default: `PipelineBuilder.__init__` の既定を `self._strict = True` に変更（未知キーは常にビルド時例外）。
- [x] C3-validate-spec-strict: `validate_spec` で各ステップのパラメータ名を関数シグネチャと突き合わせ、未知キーを `TypeError` にする（`**kwargs` 例外も撤廃）。
- [x] C4-shape-registry-clean: `api/shape_registry.py` から `CustomShape`/`ValidatedCustomShape` を削除。拡張は `shapes.base.BaseShape` + `@shapes.registry.shape` に一本化。
- [x] C5-text-arg-rename: `G.text` の引数名を `size` → `font_size` に改名し、旧名エイリアスは提供しない。
- [x] C6-stub-generator: 生成スクリプトとシェイプ署名に追随し、`python -m scripts.gen_g_stubs` で `api/__init__.pyi` を再生成。
- [x] C7-api-version: `api/__init__.py` の `__api_version__` を `"4.0"` に更新、`__breaking_changes__` に要点追記。

**リポジトリ一括更新**

- [x] R1-replace-text-args: リポジトリ内に該当呼び出しは見当たらず（N/A）。
- [x] R2-replace-displace-args: 旧引数の呼び出しはコード上で未検出（ドキュメントのレビュー記録のみ）。
- [x] R3-remove-compat-classes: 参照なしを確認、クラス削除済み。

**テスト**

- [x] T1-displace-tests: 既存テストは新シグネチャで合格。
- [x] T2-unknown-keys-fail: 既存の `validate_spec` 失敗テストが通過（厳格化により一貫）。
- [x] T3-text-font-size: `tests/test_shapes_text.py` を新規追加（`font_size` 正常系）。
- [x] T4-validate-spec-errors: `tests/test_pipeline_spec_validation.py` で網羅済み。

**ドキュメント**

- [x] D1-readme-clean: README に厳格化/キャッシュ上限を追記し、現行正規APIのみ記載。
- [x] D2-migration-table: `docs/migration.md` を追加（簡潔置換表）。
...
- [x] Q1-tests-cov: 主要スイート（noise/spec/text 等）が緑（ローカル実行）。
- [ ] Q2-mypy-precommit: mypy 実行の結果、外部依存の型スタブ欠如/attr-defined 等の課題あり（`docs/typing_report_2025-09-06.md` 参照）。pre-commit はローカルhooksで構成済み（任意実行）。
- [x] Q3-changelog: `docs/CHANGELOG.md` を追加し v4.0.0 を記載。
