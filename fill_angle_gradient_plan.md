...
- 角度グラデーションを「スキャン軸に沿った連続的な角度変化」として実装するか、「図形/グループ単位の角度補間」としてより単純なモデルにするかは要検討（表現力と実装の複雑さのバランス）。一旦は後者（図形/グループ単位の単純なモデル）で実装する。
...
- 最初の実装では仕様をシンプルに保ち、必要であれば将来のバージョンでより高度な角度制御（例: 非線形プロファイル）を検討する（この拡張は将来の実装タスクとして扱う）。
