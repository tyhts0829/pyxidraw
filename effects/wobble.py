...
def _wobble_vertices(vertices_list: list[np.ndarray], 
                    amplitude: float,
...
class Wobble(BaseEffect):
    ...
    def apply(self, vertices_list: list[np.ndarray],
        ...
        if isinstance(frequency, (int, float)):
            ...
            if len(frequency) == 3:
                freq_tuple = (float(frequency[0]), float(frequency[1]), float(frequency[2]))
            else:
                # 不正な長さの場合はデフォルト値を使用
        ...
        # wobble 関数を呼び出し
        return _wobble_vertices(vertices_list, float(amplitude), freq_tuple, float(phase))