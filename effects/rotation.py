...
class Rotation(BaseEffect):
    ...
    def apply(
        ...
        **params: Any,
        ...
        # 回転角度を抽出
        angle_x, angle_y, angle_z = rotate
        
        # エッジケース: 回転がない場合は元のデータをコピーして返す
        if abs(angle_x) < 1e-10 and abs(angle_y) < 1e-10 and abs(angle_z) < 1e-10:
            return [vertices.copy() for vertices in vertices_list]
        
        # 統合された回転行列を作成
        R = _create_rotation_matrix(angle_x, angle_y, angle_z)
