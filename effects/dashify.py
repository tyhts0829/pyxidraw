...
def _interpolate_segment(vertices: np.ndarray, cumulative_distances: np.ndarray, start_dist: float, end_dist: float) -> np.ndarray:
    ...
    if start_idx > 0 and start_dist > cumulative_distances[start_idx - 1]:
        t = (start_dist - cumulative_distances[start_idx - 1]) / (cumulative_distances[start_idx] - cumulative_distances[start_idx - 1])
    ...
    *,
    ...
    for i in range(len(offsets) - 1):
        ...
        while current_distance < total_length:
            ...
            dash_vertices = _interpolate_segment(vertices, cumulative_distances, start_distance, end_distance)
