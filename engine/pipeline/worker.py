...
class _WorkerProcess(mp.Process):
    ...
    def __init__(
        ...
        task_q: mp.Queue,
        result_q: mp.Queue,
        draw_callback: Callable[[float, Mapping[int, int]], Sequence[np.ndarray]],
    ):
    ...
    def run(self) -> None:
        ...
        for task in iter(self.task_q.get, None):  # None = sentinel
            try:
                vertices = self.draw_callback(task.t, task.cc_state)
                self.result_q.put(RenderPacket(vertices, task.frame_id))
