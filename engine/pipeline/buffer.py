...
class SwapBuffer:
    ...
    def __init__(self) -> None:
        ...
        """
        self._front: Optional[Sequence[np.ndarray]] = None
        self._back: Optional[Sequence[np.ndarray]] = None
        self._version: int = 0
        self._evt = threading.Event()
        self._lock = threading.Lock()

    # ---------- producer (BufferSubsystem) ----------
    def push(self, data: Sequence[np.ndarray]) -> None:
    ...
    def get_front(self) -> Optional[Sequence[np.ndarray]]:
        """現在の front データを取得する。"""
        return self._front
