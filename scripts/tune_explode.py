"""
Headless tuner for the `explode` effect.

Generates a PNG snapshot of a single cube placed at the canvas center
with the `explode` effect applied, so we can judge good default params.

Usage (from repo root):
    python scripts/tune_explode.py --out screenshots/explode_default.png
    python scripts/tune_explode.py --factor 0.15 --out screenshots/explode_015.png
...
try:
    ...
    with open('screenshots/tune_explode_debug.txt', 'w') as f:
        f.write('sys.path\n' + '\n'.join(sys.path) + '\n')
...
def build_geometry(factor: float) -> "Geometry":
    """Create a centered cube and apply explode(factor)."""
    ...
    pipeline = (E.pipeline.explode(factor=factor).build())
...
def main():
    ...
    p.add_argument("--factor", type=float, default=0.3, help="explode factor in [0..1]")
    p.add_argument("--out", default="screenshots/explode.png")
    p.add_argument("--scale", type=int, default=6)
    args = p.parse_args()

    geom = build_geometry(args.factor)
