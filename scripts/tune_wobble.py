"""
Headless tuner for the `wobble` effect.

Generates a PNG snapshot of a single cube placed at the canvas center
with the `wobble` effect applied, to help decide sensible defaults.

Usage (from repo root):
    python scripts/tune_wobble.py --out screenshots/wobble_default.png
    python scripts/tune_wobble.py --amplitude 2.5 --frequency 0.02 --out screenshots/wobble_2p5mm_f002.png
    python scripts/tune_wobble.py --amplitude 3.0 --frequency 0.03 0.00 0.00 --out screenshots/wobble_xyz.png
...
try:
    ...
    with open('screenshots/tune_wobble_debug.txt', 'w') as f:
        f.write('sys.path\n' + '\n'.join(sys.path) + '\n')
...
def build_geometry(amplitude: float, frequency: float | tuple[float, float, float], phase: float, subdivisions: float) -> "Geometry":
    """Create a centered cube and apply [subdivide]->wobble(amplitude, frequency, phase).

    Note: Subdivision increases vertex density so wobble shows as curves instead of parallel shifts.
    """
    ...
    pipeline = (
        ...
        .wobble(amplitude=amplitude, frequency=frequency, phase=phase)
        .build()
    )
...
def main():
    ...
    p.add_argument("--amplitude", type=float, default=2.5, help="displacement amplitude in mm (>=0)")
    p.add_argument(
        "--frequency",
        type=float,
        nargs="*",
        default=[0.02],
        help="spatial frequency (cycles per unit). Provide one value for all axes, or three for (fx fy fz)",
    )
    p.add_argument("--phase", type=float, default=0.0, help="phase in radians")
    p.add_argument("--out", default="screenshots/wobble.png")
    p.add_argument("--subdivisions", type=float, default=0.8, help="0..1 normalized; increases line vertex density before wobble")
