"""
Headless tuner for the `collapse` effect.

Places a centered cube and applies collapse(intensity, subdivisions),
exporting a PNG to help pick good defaults.
"""
...
def build_geometry(intensity: float, subdivisions: float):
    ...
    pipe = (
        E.pipeline
        .collapse(intensity=intensity, subdivisions=subdivisions)
        .build()
    )
    return pipe(cube)
...
def main():
    ...
    p.add_argument("--intensity", type=float, default=1.8)
    p.add_argument("--subdiv", type=float, default=0.5, help="0..1 normalized")
    p.add_argument("--out", default="screenshots/collapse.png")
    p.add_argument("--scale_px", type=int, default=6)
    args = p.parse_args()

    g = build_geometry(args.intensity, args.subdiv)
