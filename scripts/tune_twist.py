"""
Headless tuner for the `twist` effect.

Generates a PNG snapshot of a single cube placed at the canvas center
with the `twist` effect applied, so we can judge good default params.

Usage (from repo root):
    python scripts/tune_twist.py --out screenshots/twist_default.png
    python scripts/tune_twist.py --angle 60 --out screenshots/twist_60.png
...
# Ensure repo root is the first entry to avoid being shadowed
try:
    if str(REPO_ROOT) in sys.path:
        sys.path.remove(str(REPO_ROOT))
    sys.path.insert(0, str(REPO_ROOT))
    # Light debug to file for sandboxed environments
    os.makedirs('screenshots', exist_ok=True)
    with open('screenshots/tune_twist_debug.txt', 'w') as f:
        f.write('sys.path\n' + '\n'.join(sys.path) + '\n')
...
def build_geometry(angle: float, axis: str) -> "Geometry":
    """Create a centered cube and apply twist(angle, axis)."""
    canvas_w, canvas_h = CANVAS_SIZES["SQUARE_300"]
    cx, cy = canvas_w / 2.0, canvas_h / 2.0

    # Size: keep a comfortable margin for visual clarity
    size = min(canvas_w, canvas_h) * 0.5

    base = G.polyhedron(polygon_type="cube")
    cube = base.scale(size, size, size).translate(cx, cy, 0.0)

    # Apply the effect being tuned
    pipeline = (E.pipeline.twist(angle=angle, axis=axis).build())
...
def main():
    ...
    p.add_argument("--angle", type=float, default=45.0, help="twist angle in degrees")
    p.add_argument("--axis", choices=["x", "y", "z"], default="y")
    p.add_argument("--out", default="screenshots/twist.png")
    p.add_argument("--scale", type=int, default=6)
    args = p.parse_args()

    geom = build_geometry(args.angle, args.axis)
