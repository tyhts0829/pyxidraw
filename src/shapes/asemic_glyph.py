...
def relative_neighborhood_graph(
    nodes: list[Point3D], config: AsemicGlyphConfig
...
def random_walk_strokes_generator(
    nodes: list[Point3D],
    adjacency: dict,
...
@njit(fastmath=True, cache=True)
def _snap_point_njit(
    last_x: float, last_y: float, point_x: float, point_y: float, snap_angle: float
) -> tuple[float, float]:
    """njit化された点スナップ処理"""
    dx = point_x - last_x
    dy = point_y - last_y
...
def _compute_bezier_points_njit(
    ...
    for i in range(num_points):
        for j in range(3):
            result[i, j] = (
                one_minus_t_squared[i] * A_prime[j]
...
def smooth_polyline(
    polyline: list[Point3D], smoothing_radius: float, config: AsemicGlyphConfig
) -> list[Point3D]:
    """
    ...
    for i in range(1, len(polyline) - 1):
        ...
        bezier_points = _compute_bezier_points_njit(
            one_minus_t_squared, one_minus_t, t_values, t_squared, A_prime, B, C_prime
        )
...
def generate_nodes(
    ...
    if placement_mode == "grid":
        ...
        if config is None:
            config = AsemicGlyphConfig()
        r = (
            min(x_max - x_min, y_max - y_min) / config.poisson_radius_divisor
        )  # rを大きくするとノード数が減る
        ...
        while active_list:
            ...
            for _ in range(k):
                ...
                if (
                    candidate[0] < x_min
                    or candidate[0] > x_max
                    or candidate[1] < y_min
                    or candidate[1] > y_max
                ):
                    continue
                valid = True
