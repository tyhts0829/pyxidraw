"""
どこで: `api.effects_old`（旧 Pipeline 実装、非推奨）。
何を: 旧来の直列適用 Pipeline（インライン実行型）の参考実装を保持（互換/参考目的）。
なぜ: 新 API への移行期間における比較・回帰確認のため（将来削除予定）。

注記（Deprecated）
------------------
このモジュールは非推奨です。現行の pipeline は `api.effects` を使用してください。
本モジュールはテスト/参考目的のみで、機能追加や最適化は行いません。
