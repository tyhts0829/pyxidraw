...
class WorkerPool(Tickable):
    ...
    def __init__(
        ...
        self._workers = [
            _WorkerProcess(self._task_q, self._result_q, draw_callback) for _ in range(num_workers)
        ]
    ...
    def close(self) -> None:
        ...
        try:
            for _ in self._workers:
                # sentinel を投入（キューが満杯の可能性は低いが、ブロックせずに失敗したら諦めて強制終了経路へ）
                try:
            ...
            # close は冪等に（すでに close 済みでも例外にしない）
            try:
                self._task_q.close()
            except Exception:
                pass
