...
def _count_line(
    ...
    while u_pos < upper:
        ...
        u_start = u_pos
        u_end = u_pos + dash_len
        if u_end > upper:
            u_end = upper

        if u_end > offset and u_start < upper:
            t_start = u_start
            if t_start < offset:
                t_start = offset
            t_end = u_end
...
def dash(
    ...
    *,
    ...
    max_len = n_dash if n_dash >= n_gap else n_gap
    ...
    dash_arr = dash_arr.astype(np.float64)
    gap_arr = gap_arr.astype(np.float64)
    # offset は負値・非有限値を 0 にクランプ
    offset_arr = np.where(np.isfinite(offset_arr), offset_arr, 0.0)
    offset_arr = np.maximum(offset_arr, 0.0).astype(np.float64)
