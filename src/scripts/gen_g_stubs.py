...
Additional helper for tests/CI:
    - `generate_stubs_str()` returns the generated stub text as a string and
      installs light-weight dummies for optional heavy deps (numba, fontTools, shapely)
...
def _annotation_for_effect_param(tp: Any, imports: set[str]) -> str:
    ...
    try:
        ...
        if origin is Union:
            parts = [_annotation_for_effect_param(a, imports) for a in args]  # recurse for nested
                    ...
                    if isinstance(rules, dict):
                        ...
                        if ch is not None:
                            ...
                            if seq:
                                ...
                                parts.append(
                                    f"choices {{ {preview}{' …' if len(seq) > 6 else ''} }}"
                                )
                        if parts:
                            text = ", ".join(parts)
                if text:
...
def generate_stubs_str() -> str:
    """Return the generated api/__init__.pyi content as a string.

    Ensures shape/effect registries are populated with minimal dummy deps so this
    can run in lean CI environments and tests.
    """
    install_dummy_deps()
...
def main() -> None:
    # Use the same path as tests to ensure consistent behavior
    content = generate_stubs_str()
    # Recompute skipped list for message only
    from api.shape_registry import list_registered_shapes
