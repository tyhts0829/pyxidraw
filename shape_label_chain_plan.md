...
### ユーザーへの確認事項（この計画ファイルで相談したい点）

1. `G.text(...).label("title")` / `G.polygon(...).label("title")` のような「shape 呼び出し後の label」を正式サポートする方向で問題ないか。
2. ラベルの連番仕様について:
   - 同じ shape 名 + 同じラベル文字列で複数回呼んだ場合、ヘッダを `title`, `title_1`, `title_2` と分けたいか、それともすべて `title` にまとめたいか。
3. 型スタブの方針:
   - 最小変更（Geometry に `label` を追加）で進めるか、`LazyGeometry` を正式に型として露出させるか、どちらを優先したいか。

これらが決まり次第、このチェックリストに沿って実装・テスト・ドキュメント更新を進め、完了した項目にチェックを付けていきます。
