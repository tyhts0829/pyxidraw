...
def test_shape_decorator_supports_name_keyword() -> None:
    ...
    class CustomTestShape(BaseShape):
        def generate(self, **params: object) -> Geometry:
...
def test_shape_decorator_supports_positional_name() -> None:
    ...
    class PositionalNamedShape(BaseShape):
        def generate(self, **params: object) -> Geometry:
            return Geometry.from_lines([])
...
def test_shape_decorator_rejects_non_baseshape_with_message() -> None:
    # 非 BaseShape を登録しようとすると TypeError。
    class NotShape:  # noqa: N801 (テスト用の簡易クラス)
        pass
