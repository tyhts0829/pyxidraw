...
@pytest.mark.smoke
def test_displace_frequency_gradient_with_zero_amplitude_is_identity() -> None:
    g = Geometry.from_lines([np.array([[0.0, 0.0, 0.0], [1.0, 2.0, 3.0]], dtype=np.float32)])
    out = displace(
        g,
        amplitude_mm=0.0,
        spatial_freq=0.5,
        frequency_gradient=(1.0, -1.0, 0.5),
        t_sec=0.0,
    )
    assert np.allclose(out.coords, g.coords)
    assert np.array_equal(out.offsets, g.offsets)
...
@pytest.mark.smoke
def test_displace_frequency_gradient_zero_keeps_axis_mask_behavior() -> None:
    g = Geometry.from_lines([np.array([[0.0, 0.0, 0.0], [1.0, 2.0, 3.0]], dtype=np.float32)])
    out = displace(
        g,
        amplitude_mm=(10.0, 0.0, 0.0),
...
@pytest.mark.smoke
def test_displace_frequency_gradient_changes_sampling() -> None:
    ...
    base = displace(g, amplitude_mm=5.0, spatial_freq=0.5, frequency_gradient=0.0, t_sec=0.0)
    grad = displace(g, amplitude_mm=5.0, spatial_freq=0.5, frequency_gradient=1.0, t_sec=0.0)
    assert not np.allclose(grad.coords, base.coords)
