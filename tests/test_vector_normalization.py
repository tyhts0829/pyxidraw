...
def test_rotation_scalar_vs_tuple():
    g = _geom()
    out_scalar = rotation(g, rotate=(0.25, 0.25, 0.25))
    out_tuple = rotation(g, rotate=(0.25, 0.25, 0.25))
...
def test_transform_rotate_normalization():
    g = _geom()
    out = transform(g, rotate=(0.0, 0.0, 0.5))  # 0.5 -> pi around Z
    out2 = transform(g, rotate=(0.0, 0.0, 0.0))
    c1, _ = out.as_arrays()
    c2, _ = out2.as_arrays()
    # 異なるはず（回転あり/なし）
...
def test_array_rotate_scalar_and_vec_agree():
    g = _geom()
    out_a = array(g, n_duplicates=0.2, rotate=(0.25, 0.25, 0.25))
    out_b = array(g, n_duplicates=0.2, rotate=(0.25, 0.25, 0.25))
...
def test_translation_offset_api_equivalence():
    g = _geom()
    out_vec = translation(g, offset=(1.0, 2.0, 3.0))
    out_xyz = translation(g, offset_x=1.0, offset_y=2.0, offset_z=3.0)
