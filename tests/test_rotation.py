...
class TestRotation:
    def test_basic_rotation(self, simple_geometry):
        """基本的な回転テスト"""
        result = rotation(simple_geometry, rotate=(0.25, 0.0, 0.0))
    ...
    def test_with_center(self, simple_geometry):
        """中心点指定回転テスト"""
        center = (0.5, 0.5, 0.0)
        result = rotation(simple_geometry, center=center, rotate=(0.0, 0.0, 0.25))
        assert isinstance(result, Geometry)

    def test_multiple_axis_rotation(self, simple_geometry):
        """複数軸回転テスト"""
        result = rotation(simple_geometry, rotate=(0.1, 0.2, 0.3))
        assert isinstance(result, Geometry)
        assert len(result.coords) == len(simple_geometry.coords)

    def test_edge_case_full_rotation(self, simple_geometry):
        """1回転テスト（360度 = 1.0）"""
        result = rotation(simple_geometry, rotate=(0.0, 0.0, 1.0))
        assert isinstance(result, Geometry)
    ...
    def test_negative_rotation(self, simple_geometry):
        """負の回転テスト"""
        result = rotation(simple_geometry, rotate=(0.0, 0.0, -0.25))
        assert isinstance(result, Geometry)

    def test_large_rotation(self, simple_geometry):
        """大きな回転値テスト"""
        result = rotation(simple_geometry, rotate=(5.0, 3.0, 2.0))
        assert isinstance(result, Geometry)
