...
class TestExtrude:
    def test_no_extrusion(self, simple_geometry):
        ...
        result = extrude(simple_geometry, distance=0.0)
        assert isinstance(result, Geometry)

    def test_basic_extrusion(self, square_geometry):
        """基本的な押し出しテスト"""
        result = extrude(square_geometry, distance=0.5, direction=(0, 0, 1))
        assert isinstance(result, Geometry)
        # 押し出しにより線分が増加
    ...
    def test_z_direction_extrusion(self, square_geometry):
        """Z方向押し出しテスト"""
        result = extrude(square_geometry, distance=0.8, direction=(0, 0, 1))
        assert isinstance(result, Geometry)

    def test_x_direction_extrusion(self, square_geometry):
        """X方向押し出しテスト"""
        result = extrude(square_geometry, distance=0.6, direction=(1, 0, 0))
        assert isinstance(result, Geometry)

    def test_diagonal_direction_extrusion(self, square_geometry):
        """対角線方向押し出しテスト"""
        result = extrude(square_geometry, distance=0.5, direction=(1, 1, 1))
        assert isinstance(result, Geometry)

    def test_scaling_during_extrusion(self, square_geometry):
        """押し出し時のスケーリングテスト"""
        result = extrude(square_geometry, distance=0.5, scale=0.3)
        assert isinstance(result, Geometry)

    def test_subdivisions_during_extrusion(self, square_geometry):
        """押し出し時の細分化テスト"""
        result = extrude(square_geometry, distance=0.5, subdivisions=0.8)
        assert isinstance(result, Geometry)

    def test_max_distance(self, square_geometry):
        """最大距離押し出しテスト"""
        result = extrude(square_geometry, distance=1.0)
        assert isinstance(result, Geometry)

    def test_min_scale(self, square_geometry):
        """最小スケール押し出しテスト"""
        result = extrude(square_geometry, distance=0.5, scale=0.0)
        assert isinstance(result, Geometry)

    def test_max_scale(self, square_geometry):
        """最大スケール押し出しテスト"""
        result = extrude(square_geometry, distance=0.5, scale=1.0)
        assert isinstance(result, Geometry)

    def test_max_subdivisions(self, square_geometry):
        """最大細分化押し出しテスト"""
        result = extrude(square_geometry, distance=0.5, subdivisions=1.0)
        assert isinstance(result, Geometry)
    ...
    def test_simple_line_extrusion(self, simple_geometry):
        """単純な線の押し出しテスト"""
        result = extrude(simple_geometry, distance=0.5)
        assert isinstance(result, Geometry)
