...
class TestRepeat:
    def test_no_duplicates(self, simple_geometry):
        ...
        result = repeat(simple_geometry, n_duplicates=0.0)
    ...
    def test_basic_array(self, simple_geometry):
        """基本的な配列テスト"""
        result = repeat(simple_geometry, n_duplicates=0.5, offset=(10, 0, 0))
        assert isinstance(result, Geometry)
        # 複製により頂点数が増加
        assert len(result.coords) >= len(simple_geometry.coords)
    ...
    def test_max_duplicates(self, simple_geometry):
        ...
        result = repeat(simple_geometry, n_duplicates=1.0, offset=(5, 5, 0))
        assert isinstance(result, Geometry)

    def test_x_offset_only(self, simple_geometry):
        """X軸オフセットのみテスト"""
        result = repeat(simple_geometry, n_duplicates=0.3, offset=(10, 0, 0))
        assert isinstance(result, Geometry)

    def test_y_offset_only(self, simple_geometry):
        """Y軸オフセットのみテスト"""
        result = repeat(simple_geometry, n_duplicates=0.3, offset=(0, 10, 0))
        assert isinstance(result, Geometry)

    def test_z_offset_only(self, simple_geometry):
        """Z軸オフセットのみテスト"""
        result = repeat(simple_geometry, n_duplicates=0.3, offset=(0, 0, 10))
        assert isinstance(result, Geometry)

    def test_diagonal_offset(self, simple_geometry):
        """対角線オフセットテスト"""
        result = repeat(simple_geometry, n_duplicates=0.4, offset=(5, 5, 5))
        assert isinstance(result, Geometry)
    ...
    def test_negative_offset(self, simple_geometry):
        """負のオフセットテスト"""
        result = repeat(simple_geometry, n_duplicates=0.3, offset=(-5, -3, -2))
        assert isinstance(result, Geometry)

    def test_zero_offset(self, simple_geometry):
        """ゼロオフセットテスト"""
        result = repeat(simple_geometry, n_duplicates=0.3, offset=(0, 0, 0))
        assert isinstance(result, Geometry)

    def test_large_offset(self, simple_geometry):
        """大きなオフセットテスト"""
        result = repeat(simple_geometry, n_duplicates=0.3, offset=(100, 200, 50))
        assert isinstance(result, Geometry)

    def test_various_duplicate_counts(self, simple_geometry):
        """様々な複製数テスト"""
        for n_duplicates in [0.1, 0.3, 0.5, 0.7, 0.9]:
            result = repeat(simple_geometry, n_duplicates=n_duplicates, offset=(5, 0, 0))
            assert isinstance(result, Geometry)

    def test_rotation_variations(self, simple_geometry):
        """回転バリエーションテスト"""
        # X軸回転
        result_x = repeat(simple_geometry, n_duplicates=0.3, offset=(5, 0, 0), rotate=(0.2, 0.0, 0.0))
        # Y軸回転
        result_y = repeat(simple_geometry, n_duplicates=0.3, offset=(5, 0, 0), rotate=(0.0, 0.2, 0.0))
        # Z軸回転
        result_z = repeat(simple_geometry, n_duplicates=0.3, offset=(5, 0, 0), rotate=(0.0, 0.0, 0.2))
    ...
    def test_scale_variations(self, simple_geometry):
        """スケールバリエーションテスト"""
        # 拡大
        result_expand = repeat(simple_geometry, n_duplicates=0.3, offset=(5, 0, 0), scale=(0.8, 0.8, 0.8))
        # 縮小
        result_shrink = repeat(simple_geometry, n_duplicates=0.3, offset=(5, 0, 0), scale=(0.2, 0.2, 0.2))
