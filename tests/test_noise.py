...
class TestNoise:
    ...
    def test_time_parameter(self, simple_geometry):
        ...
        result1 = effect.apply(simple_geometry, intensity=0.5, time=0.0)
        result2 = effect.apply(simple_geometry, intensity=0.5, time=1.0)
    ...
    def test_reproducibility(self, simple_geometry):
        ...
        # 同じパラメータで複数回実行
        result1 = effect.apply(simple_geometry, intensity=0.5, frequency=0.3, time=0.5)
        result2 = effect.apply(simple_geometry, intensity=0.5, frequency=0.3, time=0.5)
    ...
    def test_negative_time(self, simple_geometry):
        """負の時間テスト"""
        effect = Noise()
        result = effect.apply(simple_geometry, intensity=0.5, time=-1.0)
        assert isinstance(result, Geometry)

    def test_large_time(self, simple_geometry):
        """大きな時間値テスト"""
        effect = Noise()
        result = effect.apply(simple_geometry, intensity=0.5, time=1000.0)
        assert isinstance(result, Geometry)
