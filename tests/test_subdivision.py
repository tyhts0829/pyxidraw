...
class TestSubdivision:
    def test_no_subdivision(self, simple_geometry):
        ...
        result = subdivide(simple_geometry, subdivisions=0.0)
    ...
    def test_basic_subdivision(self, simple_geometry):
        """基本的な細分化テスト"""
        result = subdivide(simple_geometry, subdivisions=0.5)
        assert isinstance(result, Geometry)
        # 細分化により頂点数が増加することを確認
        assert len(result.coords) >= len(simple_geometry.coords)
    ...
    def test_max_subdivision(self, simple_geometry):
        ...
        result = subdivide(simple_geometry, subdivisions=1.0)
        assert isinstance(result, Geometry)
        # 最大細分化で頂点数が大幅に増加
        assert len(result.coords) >= len(simple_geometry.coords)
    ...
    def test_increasing_subdivision(self, simple_geometry):
        """段階的な細分化テスト"""
        result_low = subdivide(simple_geometry, subdivisions=0.2)
        result_mid = subdivide(simple_geometry, subdivisions=0.5)
        result_high = subdivide(simple_geometry, subdivisions=0.8)
    ...
    def test_preserves_line_count(self, simple_geometry):
        """線分数保持テスト"""
        result = subdivide(simple_geometry, subdivisions=0.7)
        assert isinstance(result, Geometry)
        # 線分の数は同じ
    ...
    def test_edge_values(self, simple_geometry):
        """境界値テスト"""
        # 最小値
        result_min = subdivide(simple_geometry, subdivisions=0.0)
        assert isinstance(result_min, Geometry)
        
        # 最大値
        result_max = subdivide(simple_geometry, subdivisions=1.0)
        assert isinstance(result_max, Geometry)

    def test_intermediate_values(self, simple_geometry):
        """中間値テスト"""
        for value in [0.1, 0.3, 0.6, 0.9]:
            result = subdivide(simple_geometry, subdivisions=value)
            assert isinstance(result, Geometry)
            assert len(result.coords) >= len(simple_geometry.coords)
    ...
    def test_single_point_line(self):
        ...
        result = subdivide(geometry, subdivisions=0.5)
        assert isinstance(result, Geometry)
        # 単一点の場合は変化なし
