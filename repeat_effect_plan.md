...
- [ ] A-1. 新規引数名を決める
  - 候補: `cumulative`, `accumulate`, `from_origin`
  - 暫定案: `cumulative: bool = True`（現行挙動をデフォルトとして維持）
- [ ] A-2. 累積モード時の仕様を整理
  - 現行挙動をそのまま維持:
...
- [ ] A-3. 非累積モード時の仕様を整理
  ...
  - 各コピー n（0-origin）について:
    ...
    - スケールの扱いは要検討:
      - 案1: 累乗 (`scale ** (n + 1)`) でサイズだけはステップごとに変化させる。
      - 案2: 毎回同じ `scale` を適用し、全コピー同一サイズにする。
- [ ] A-4. 既存スケッチへの互換性方針を明文化
  - 新規引数の既定値で「現行挙動と完全互換」にする。
  - 既存コードには変更不要とする。
- [ ] A-5. docstring にどう記述するかの方針決定
  - 先頭行は現行の概要を維持。
...
- [ ] C-1. `repeat` 関数の docstring を更新
  - Parameters に新規 bool 引数を追加。
  - 累積/非累積モードの違いを 1〜2 行で説明。
- [ ] C-2. `repeat.__param_meta__` に bool パラメータを追加
  - 例: `"cumulative": {"type": "bool"}`。
  - レンジ指定は不要（bool のため）。
- [ ] C-3. docstring と `__param_meta__` の既定値/意味が一致しているか確認。
...
- [ ] D-1. `tests/effects/test_repeat_basic.py`（仮）を新規作成
  - 累積モードでの位置/スケールが現行と同じことを確認するテスト。
  - 非累積モードで「等間隔に並ぶ」ことを確認するテスト。
- [ ] D-2. `count=0` / 入力空ジオメトリなど、no-op 条件のテストを追加。
