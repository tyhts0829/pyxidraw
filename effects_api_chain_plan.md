...
- [ ] B-1. 現行 `_EffectsAPI` と `PipelineBuilder` の責務を再確認
...
- [ ] C-1. `tools/gen_g_stubs.py` の `_render_pipeline_protocol` を読み直す
  - 現状: `_PipelineBuilder` に effect メソッド、`_Effects` に `pipeline()` のみ生成
- [ ] C-2. `_Effects` Protocol にも effect メソッドを生やす方針を決める
...
- [ ] C-4. スタブ生成スクリプトを修正し、`python -m tools.gen_g_stubs` で `src/api/__init__.pyi` を再生成
- [ ] C-5. スタブ同期テストの確認
...
- [ ] E-4. 既存 `E.pipeline` テストがすべて通ることを確認
