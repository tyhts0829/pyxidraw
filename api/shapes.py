...
def polygon(n_sides: int | float = 3, 
...
def sphere(subdivisions: float = 0.5,
           center: tuple[float, float, float] = (0, 0, 0),
           scale: tuple[float, float, float] = (1, 1, 1),
           rotate: tuple[float, float, float] = (0, 0, 0),
           **params: Any) -> list[np.ndarray]:
...
def grid(n_divisions: tuple[float, float] = (0.1, 0.1),
         center: tuple[float, float, float] = (0, 0, 0),
         scale: tuple[float, float, float] = (1, 1, 1),
         rotate: tuple[float, float, float] = (0, 0, 0),
         **params: Any) -> list[np.ndarray]:
...
def polyhedron(polygon_type: str | int = "tetrahedron",
               center: tuple[float, float, float] = (0, 0, 0),
               scale: tuple[float, float, float] = (1, 1, 1),
               rotate: tuple[float, float, float] = (0, 0, 0),
               **params: Any) -> list[np.ndarray]:
...
def lissajous(freq_x: float = 3.0, freq_y: float = 2.0, 
              phase: float = 0.0, points: int = 1000,
              center: tuple[float, float, float] = (0, 0, 0),
              scale: tuple[float, float, float] = (1, 1, 1),
              rotate: tuple[float, float, float] = (0, 0, 0),
              **params: Any) -> list[np.ndarray]:
    ...
    return shape(freq_x=freq_x, freq_y=freq_y, phase=phase, points=points, 
                center=center, scale=scale, rotate=rotate, **params)


def torus(major_radius: float = 0.3, minor_radius: float = 0.1,
          major_segments: int = 32, minor_segments: int = 16,
          center: tuple[float, float, float] = (0, 0, 0),
          scale: tuple[float, float, float] = (1, 1, 1),
          rotate: tuple[float, float, float] = (0, 0, 0),
          **params: Any) -> list[np.ndarray]:
    ...
    return shape(major_radius=major_radius, minor_radius=minor_radius,
                major_segments=major_segments, minor_segments=minor_segments,
                center=center, scale=scale, rotate=rotate, **params)


def cylinder(radius: float = 0.3, height: float = 0.6, 
             segments: int = 32,
             center: tuple[float, float, float] = (0, 0, 0),
             scale: tuple[float, float, float] = (1, 1, 1),
             rotate: tuple[float, float, float] = (0, 0, 0),
    ...
    return shape(radius=radius, height=height, segments=segments,
                center=center, scale=scale, rotate=rotate, **params)


def cone(radius: float = 0.3, height: float = 0.6,
         segments: int = 32,
         center: tuple[float, float, float] = (0, 0, 0),
         scale: tuple[float, float, float] = (1, 1, 1),
         rotate: tuple[float, float, float] = (0, 0, 0),
...
def capsule(radius: float = 0.2, height: float = 0.4,
            segments: int = 32,
            center: tuple[float, float, float] = (0, 0, 0),
            scale: tuple[float, float, float] = (1, 1, 1),
            rotate: tuple[float, float, float] = (0, 0, 0),
...
def attractor(attractor_type: str = "lorenz", points: int = 10000, 
              dt: float = 0.01,
              center: tuple[float, float, float] = (0, 0, 0),
              scale: tuple[float, float, float] = (1, 1, 1),
              rotate: tuple[float, float, float] = (0, 0, 0),
    ...
    return shape(attractor_type=attractor_type, points=points, dt=dt,
                center=center, scale=scale, rotate=rotate, **params)


def text(text: str = "HELLO", size: float = 0.1,
         center: tuple[float, float, float] = (0, 0, 0),
         scale: tuple[float, float, float] = (1, 1, 1),
         rotate: tuple[float, float, float] = (0, 0, 0),
         **params: Any) -> list[np.ndarray]:
...
def asemic_glyph(complexity: int = 5, seed: int | None = None,
                 center: tuple[float, float, float] = (0, 0, 0),
                 scale: tuple[float, float, float] = (1, 1, 1),
                 rotate: tuple[float, float, float] = (0, 0, 0),
                 **params: Any) -> list[np.ndarray]:
